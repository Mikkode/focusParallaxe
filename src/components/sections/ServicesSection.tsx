    ...
    <div ref={sectionRef} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {services.map((service, index) => (
          <div 
            key={index} 
            className="p-6 rounded-xl shadow-sm bg-white border border-slate-100 hover:shadow-md transition-all duration-300 group animate-on-scroll opacity-0 transition-all duration-700"
            style={{ transitionDelay: `${150 + index * 100}ms` }}
          >
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-sky-100 mb-6 group-hover:bg-sky-500 transition-all duration-300">
              <div className="text-sky-600 group-hover:text-white transition-all duration-300">
                {service.icon}
              </div>
            </div>
            <h3 className="text-xl font-semibold text-slate-800 mb-3 group-hover:text-sky-500 transition-all duration-300">{service.title}</h3>
